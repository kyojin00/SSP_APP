...
class _MealTodayTabState extends State<MealTodayTab> {
  ...
  Future<void> _loadGuests() async {
    try {
      final data = await supabase.from('meal_guests').select()
          .eq('meal_date', _today);
      if (mounted) setState(() =>
          _guests = List<Map<String, dynamic>>.from(data));
    } catch (e) { debugPrint('손님 로드 실패: $e'); }
  }
  ...
  void _showDeptDetail(
    ...
    final dc         = mrDeptColor(dept);
    final deptRows   = allRows
        .where((r) => r['dept_category'] == dept).toList();
    final eating     = deptRows
        .where((r) => r['is_eating'] == true).toList();
    final notEating  = deptRows
        .where((r) => r['is_eating'] == false).toList();
    final deptProfiles = _filteredProfiles
        .where((p) => p['dept_category'] == dept).toList();
    final repliedIds = deptRows
        .map((r) => r['user_id'] as String).toSet();
              ...
              child: ListView(
                ...
                children: [
                  if (eating.isNotEmpty) ...[
                    ...
                    ...eating.map((r) => _deptPersonTile(
                        r['full_name'] ?? '-', mrOrange,
                        icon: Icons.check_circle_rounded)),
                  ],
                  if (notEating.isNotEmpty) ...[
                    _deptSectionHeader(
                        '🚫 불참', notEating.length, mrSub),
                    ...notEating.map((r) => _deptPersonTile(
                        r['full_name'] ?? '-', mrSub,
                        icon: Icons.cancel_rounded)),
                  ],
                  if (noReply.isNotEmpty) ...[
                    _deptSectionHeader(
                        '❓ 미응답', noReply.length, mrRed),
                    ...noReply.map((p) => _deptPersonTile(
                        p['full_name'] ?? '-', mrRed,
                        icon: Icons.help_rounded,
                        nationality:
                            p['nationality'] as String?)),
                  ],
                ],
              ),
            ),
      ...
      builder: (context, snap) {
        ...
        final todayAll = raw
            .where((r) =>
                ...
                !_excludedFromMeal.contains(r['dept_category']))
            .toList();
        final lunchRows  = todayAll
            .where((r) => r['meal_type'] == 'LUNCH').toList();
        final dinnerRows = todayAll
            .where((r) => r['meal_type'] == 'DINNER').toList();

        final lunchGuests = _guests
            .where((g) => g['meal_type'] == 'LUNCH')
            .fold(0, (s, g) =>
                s + ((g['guest_count'] as int?) ?? 0));
        final dinnerGuests = _guests
            .where((g) => g['meal_type'] == 'DINNER')
            .fold(0, (s, g) =>
                s + ((g['guest_count'] as int?) ?? 0));

        MealStat makeTotalStat(
            List<Map<String, dynamic>> rows, int guests) {
          final e = rows
                  .where((r) => r['is_eating'] == true).length +
              guests;
          final n =
              rows.where((r) => r['is_eating'] == false).length;
          return MealStat(
            eating: e,
            notEating: n,
            noReply: (_filteredTotalMembers -
                    rows
                        .where((r) => r['is_eating'] == true)
                        .length -
                    n)
                .clamp(0, 99999),
        ...
        List<Map<String, dynamic>> withNat(
        ...
        List<Map<String, dynamic>> eating(
                List<Map<String, dynamic>> rows) =>
            withNat(rows
                .where((r) => r['is_eating'] == true)
                .toList());
        List<Map<String, dynamic>> notEating(
                List<Map<String, dynamic>> rows) =>
            withNat(rows
                .where((r) => r['is_eating'] == false)
                .toList());
        List<Map<String, dynamic>> noReply(
            List<Map<String, dynamic>> rows) {
          final repliedIds = rows
              .map((r) => r['user_id'] as String)
              .toSet();
          return _filteredProfiles
              .where((p) =>
                  !repliedIds.contains(p['id'] as String))
              .toList();
        }

        List<DeptMealStat> makeDeptStats(
            List<Map<String, dynamic>> rows) {
          return _filteredDepts.map((dept) {
            final members = _filteredProfiles
                ...
                .length;
            final dr = rows
                .where((r) => r['dept_category'] == dept)
                .toList();
            final e =
                dr.where((r) => r['is_eating'] == true).length;
            final n =
                dr.where((r) => r['is_eating'] == false).length;
            return DeptMealStat(
              dept: dept, members: members, eating: e,
              notEating: n,
        ...
        final dateLabel = DateFormat('MM월 dd일 (E)', 'ko_KR')
            .format(DateTime.now());
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(
                          color: mrOrange.withOpacity(0.3)),
                    ),
                    child: Row(children: [
  ...
  Widget _mealSectionCard(
    ...
    String _natSub(List<Map<String, dynamic>> list,
        {int guests = 0}) {
      ...
      final foreignTotal =
          natCount.values.fold(0, (a, b) => a + b);
      final domestic = list.length + guests - foreignTotal;
      final flags = natCount.entries.map((e) {
        final flag =
            e.key == '손님' ? '👤' : _nationalityFlag(e.key);
        return '$flag${e.value}';
      }).join(' ');
      return domestic > 0
          ? '$domestic+$foreignTotal  $flags'
          : flags;
    }

    final lighter = HSLColor.fromColor(color)
        .withLightness(
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              _tapBox(context,
                ...
                persons: eatingList,
                nationalitySub: _natSub(eatingList,
                    guests: guestCount),
          ...
          ...deptStats.asMap().entries.map((entry) {
            ...
            final r2 = s.members > 0
                ? (s.eating + s.notEating) / s.members
                : 0.0;
    ...
    return Expanded(
      child: GestureDetector(
        onTap: () => _showPersonList(context,
            title: sheetTitle, color: color, icon: icon,
            persons: persons),
        child: Container(
          padding: const EdgeInsets.symmetric(vertical: 10),
          decoration: BoxDecoration(
          ...
          child: Column(children: [
            ...
            if (nationalitySub != null &&
                nationalitySub.isNotEmpty) ...[
              const SizedBox(height: 1),
              Text(nationalitySub,
                  style: TextStyle(
                      fontSize: 10,
  ...
  Widget _deptSectionHeader(String title, int count, Color color) =>
      Padding(
  ...
  Widget _deptPersonTile(String name, Color color,
    ...
    final flag = nationality != null &&
            nationality.isNotEmpty &&
            nationality != '한국'
        ? _nationalityFlag(nationality)
        : '';
    return Container(
